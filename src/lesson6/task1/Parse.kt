...
fun bestLongJump(jumps: String): Int {
    val space = jumps.split(" ")
    val rubbish = setOf(" ", "%", "-")
    if (jumps.matches(Regex("""[\-%\s]+"""))) return -1
    if (jumps.matches(Regex("""[\d\s\-%]+"""))) {
        return space.filter {it !in rubbish}.map { it.toInt() }.max()
    }
    return -1
}
...
fun mostExpensive(description: String): String {
    var cost = 0.0
    var answer = ""
    if (!description.matches(Regex("""([А-яA-z]+ [\d.]+; )*[А-яA-z]+ [\d.]+"""))) return ""
