...
fun subtractOf(a: MutableMap<String, String>, b: Map<String, String>) {
    b.forEach { (s, i) ->
        if (a.containsValue(i) && a.containsKey(s) && a[s] == i) {
            a.remove(s)
        }
    }
}
...
fun mergePhoneBooks(mapA: Map<String, String>, mapB: Map<String, String>): Map<String, String> {
    var new = mutableMapOf<String, String>()
    new = ((mapA + mapB) as MutableMap<String, String>)
    mapA.forEach { (s, i) ->
        if (mapB.containsKey(s) && mapB[s] != i) {
            new[s] = mapA[s] + ", " + mapB[s]
        }
    }
    return new
...
fun canBuildFrom(chars: List<Char>, word: String): Boolean {
    ...
    var chars2 = chars.toSet()
    chars2 = chars2.map { it.lowercaseChar() }.toSet()
    for (i in newWord) {
        for (it in chars2) {
            if (i == it) {
                count++
